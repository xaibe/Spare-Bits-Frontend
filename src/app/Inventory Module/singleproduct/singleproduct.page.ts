...
export class SingleproductPage implements OnInit {
  ...
  showseller(email) {
    ...
    this.router.navigateByUrl("/usersprofile");
  }
  ...
  async sendfeedback(pro) {
    if (pro.email === this.email) {
      ...
      console.log(
        "send feedback text area value",
        this.FeedbackForm.controls["Feedback"].value
      );
      ...
      let fedbackarray: MyObject[] = [
        {
          name: this.name,
          email: this.email,
          feed: this.FeedbackForm.controls["Feedback"].value,
        },
      ];
      ...
      const observable = await this.productsService.feedback(
        this.proname,
        fedbackarray
      );
